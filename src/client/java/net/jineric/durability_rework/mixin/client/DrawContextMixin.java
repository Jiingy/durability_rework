...
import net.jineric.durability_rework.StaticMixinFields;
...
public abstract class DrawContextMixin {
	...
	private boolean applyItemLevelBarMode(ItemStack instance, Operation<Boolean> original) {
		if (instance.durability$isUpgradable() && this.canShowLevelBar(instance) && this.client.player != null) {
			ClientPlayerEntity clientPlayer = this.client.player;
			switch (DrGameOptions.getItemLevelBarMode().getValue()) {
...
//							|| instance == this.client.player.getActiveItem()
	...
	private boolean canShowLevelBar(ItemStack itemStack) {
		return itemStack.durability$getLevel() >= 1 && itemStack.durability$getLevel() < itemStack.durability$getMaxLevel();
	}
}
